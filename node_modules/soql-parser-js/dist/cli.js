...
const printJson = argv.json || argv.j;
log({ data: JSON.stringify(argv, null, 2) });
function log(options) {
    if (debug) {
        print(Object.assign({}, options, { debug: true }));
    }
}
    ...
    if (!options.query
        ...
        try {
            if (fs.existsSync(options.query)) {
                query = fs.readFileSync(options.query, 'utf8');
                ...
                if (!query
                    ...
                    print({
                        error: true,
                        message: `The query contained within the file ${options.query} does not appear to be valid, please make sure the query starts with SELECT.`,
                    });
                    process.exit(1);
                }
            }
            ...
            print({
                error: true,
                message: `There was an error parsing the file ${options.query}. Please ensure the file exists and is a text file containing a single SOQL query.`,
            });
...
function composeQuery$1(options) {
    // if query starts with SELECT we know it is not a file, otherwise we will attempt to parse a file
    // Check if query does not look like a query - attempt to parse file if so
    ...
    if (!options.compose.trim().startsWith('{')) {
        ...
        try {
            if (fs.existsSync(options.compose)) {
                parsedQueryString = fs.readFileSync(options.compose, 'utf8');
                log({
                ...
                print({
                    error: true,
                    message: `The file ${options.compose} does not exist, Please provide a valid filepath or an escaped JSON string.`,
                });
                process.exit(1);
            }
        }
            ...
            print({
                error: true,
                message: `There was an error reading the file ${options.compose}. Please ensure the file exists and is a text file containing a single parsed query JSON object.`,
            });
            log({ error: true, data: ex });
            process.exit(1);
        }
...
function printHelp() {
    const help = [
        ...
        {
            param: '--compose, -c',
            message: 'An escaped and quoted parsed SOQL JSON string or a file path to a text file containing a parsed query JSON object.',
        },
    ...
    print({
        message: 'To use the CLI, provide one or more of the following commands. Either one of the  query or compose method are required',
    });
    help.forEach(item => {
